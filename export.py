...
def build_affine_matrix(matrix_node, add_node, prev_node_name, checkpoint_reader):
  ...
  fc_array = {'layer_type': 'fc',
               'out_depth': variable_tensor.shape[1],
               'out_sx': 1,
               'out_sy': 1,
               'biases': array_to_json(bias_tensor),
               'num_inputs': variable_tensor.shape[0],
               'filters': []}
  for col_index in range(variable_tensor.shape[1]):
    fc_array['filters'].append(array_to_json(variable_tensor[:,col_index]))
...
def generate_model_json(args):
  ...
  saver.restore(sess, tf.train.latest_checkpoint(args.checkpoint_file[0]))

  graph_def = tf.get_default_graph().as_graph_def(add_shapes=True)
  graph = graph_util.TFGraph(graph_def)
  reader = tf.train.NewCheckpointReader(tf.train.latest_checkpoint(args.checkpoint_file[0]))
  ...
  while marker < len(path):
    ...
    if node_proto.op == 'Placeholder':
      ...
      if marker < len(path) - 1 and graph.get_node(path[marker+1]).op == 'BiasAdd':
